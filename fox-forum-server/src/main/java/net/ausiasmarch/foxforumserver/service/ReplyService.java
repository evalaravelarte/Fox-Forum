...
public class ReplyService {
    ...
    public ReplyEntity get(long id) {
        return oReplyRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Reply not found")); //encontrar un registro, si no lanza excepcion
    }
    ...
    public ReplyEntity update (ReplyEntity oReplyEntity) {
        ReplyEntity oReplyEntityAux = oReplyRepository.findById(oReplyEntity.getId()).orElseThrow(() -> new ResourceNotFoundException("Reply not found"));
    ...
    public ReplyEntity delete (Long id) {
        ReplyEntity oReplyEntityAux = oReplyRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Reply not found"));
    ...
    public Long populate (Integer amount) {
        for(int i=0;i < amount;i++) {
            oReplyRepository.save(new ReplyEntity("title" + i, "post" + i, (long) i, (long) i));
        }
